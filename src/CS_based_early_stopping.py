...
if __name__ == '__main__':
    ...
    if feature_type == 'answer':
        ...
        feature_li = ['QUA_IM', 'DIF_IV', 'SIM_COT_AGG', 'SIM_AC_BIGRAM', 'SIM_AC_AGG', 'SIM_AC_PW']
    
    # Train LR model on train set, tune on val set, get scores for test set only
    df_test_with_scores, best_threshold, trained_model = trained_LR_model_with_splits(
        df_train, df_val, df_test, feature_li
    )
    
    # Get parameters from command line
    threshold = float(sys.argv[1]) if len(sys.argv) > 1 else best_threshold
    N = int(sys.argv[2]) if len(sys.argv) > 2 else 5
    stop_mechanism = str(sys.argv[3]) if len(sys.argv) > 3 else 'PositiveN'
    
    print(f"\nUsing threshold: {threshold} (best from val: {best_threshold:.4f})")
    print(f"Using buffer size N: {N}")
    print(f"Using stop mechanism: {stop_mechanism}")
    
    # Run early stopping ONLY on test set
    df_results, metrics = CS_early_stopping(
        df=df_test_with_scores, 
        threshold=threshold, 
        N=N, 
